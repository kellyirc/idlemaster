...
public class Player extends Playable{
	...
	public void takeTurn() {
		...
		stats.timeSpent = stats.timeSpent.add(new BigInteger("1"));
		
		if(stats.timeSpent.longValue() % 50000 == 0) {
			addNewItem();
		}
	}
	...
	private void sell(Item i) {
		int gain = 0;
		gain += i.getValue()/4;
		gain *= Math.min(level/10,1);
		if(gain > 0) {
			if(!isIgnoring) {
	...
	private void equip(Slot s, Item i) {
